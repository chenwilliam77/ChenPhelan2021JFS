...
s.rho = .04; s.a = 1; s.a_= 0.85 * s.a;

s.delta = .05;
s.lambda = 1 / 9.6958; % inverse to get the right price
s.sigA = .02; s.sigB = .02;
s.gammaA = 1.05; s.gammaB = 2;
% s.gammaA = 1.1; s.gammaB = 1.1;
s.alpha = 0.5; s.psi = 1;
s.tau = 0;
s.LA = 1e3;
s.LB = 1e3; % s.LA = 2; % leverage constraint for A
% s.LB = 1; % leverage constraint for B
% legend({'L_A = 5', 'L_A = 1', 'L_A = .5'}, 'FontSize', 16);
% Settings for numerical solution method
set(groot, 'defaultLineLineWidth', 3);
s.N = 100; s.twist = 1.01; s.start = 1e-2; s.end = 1 - 1e-2;
