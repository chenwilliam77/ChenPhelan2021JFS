function display_results(grid, welf, stat, line_type, diagnostics, pareto_weight)
...
figure(21);
plot(grid.eta, grid.eta .* grid.mu_eta, line_type); hold on
xlabel('\eta');
title('Drift of \eta');

figure(22);
plot(grid.eta, grid.eta .* sqrt(grid.sig_etaA.^2 + grid.sig_etaB.^2), line_type); hold on
xlabel('\eta');
title('Volatility of \eta');

figure(23);
plot(grid.eta, grid.q .* sqrt(grid.sig_qA.^2 + grid.sig_qB.^2), line_type); hold on
xlabel('\eta');
title('Volatility of q');

figure(24);
plot(grid.eta(2:end-1), stat.pdf(2:end-1), line_type); hold on
xlabel('\eta');
